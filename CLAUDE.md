...
- **Entities**: `User` (name, email_address, password_digest, role), `Portal` (name, dimension, departure_time, capacity), `Booking` (user_id, portal_id), `Activity` (user_id, user_name, action, details). `User` also has an `avatar` (a key from the fixed set `User::AVATARS`, picture in `app/assets/images/<key>.png`, no uploads). Free seats = `capacity` minus the number of bookings; there is no stored counter.
...
- **Live updates**: Turbo Streams over Action Cable, but only a "look again" signal, never HTML (ADR-0003). Models declare `refreshes_pages_on "<stream>"` (`LiveUpdates` concern), pages that show data call `live_updates "<stream>"`, forms and the profile never do. Streams: `portals` (Portal and Booking), `users`, `activities`. Only logged-in Users can connect (`ApplicationCable::Connection`). Proof with two real browsers: `node script/live_check.mjs`.
- **Images**: the user's own artwork lives in `app/assets/images/` (portal.gif, portal.png as logo, gun.png, rick1.gif, rick2.gif and one PNG per avatar). The GIFs must carry a loop flag or they play once and freeze (fixed losslessly, see git history). The static error pages in `public/` cannot use the asset pipeline, so the Rick GIFs are copied to `public/bilder/`. The favicon is `public/icon.png`.
