...
- **Signature element:** the portal. First a swirling green portal built from CSS gradients; it was later replaced by the project's own portal GIF on the login page (with the portal gun in front) and on the Portal details page, and by `portal.png` as the logo.
